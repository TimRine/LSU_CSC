...
class Scanner 
{
	...
	try 
		...
		if (ch == -1) 
                {
                    System.err.println("Error: unexpected end of file after #");
                    return null;
		}
		System.err.println("Error: illegal character '" + ch + "' after #");
		return getNextToken();
            }
		...
		return new IdentToken(new String(this.buf, 0, i - 1));
            ...
            System.err.println("Error: Illegal input character '" + ch + '\'');
		return getNextToken();
	} catch (IOException e) 
        {
